            ...
            <table style={{ borderCollapse: "collapse", width: "100%", fontSize: 13 }}>
              ...
              <tbody>
                {BRAND_GROUPS.map((groupName, bi) => {
                  ...
                  // For merged brands, primary brand is the first one (ยูนิค)
                  const primaryBrand = subBrands[0];
                  const bt = isMerged ? mergedBrandTotal(subBrands) : brandTotal(primaryBrand);
                  return (
                    <>
                      {WEIGHTS.map((w, wi) => {
                        // For merged: show combined sum; +/- and edit go to primaryBrand
                        const row = isMerged ? mergedCell(subBrands, w) : getCell(primaryBrand, w);
                        const primaryRow = getCell(primaryBrand, w);
                        const total = totalRow(row);
                        return (
                          <tr key={`${groupName}-${w}`} style={{ background: (bi + wi) % 2 === 0 ? WHITE : LIGHT }}>
                              ...
                              <td key={f.key} style={{ padding: "4px 6px", textAlign: "center" }}>
                                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 3 }}>
                                  ...
                                  <input
                                    ...
                                    key={`${groupName}-${w}-${f.key}-${row[f.key] || 0}`}
                                    onBlur={async e => {
                                  ...
                                  />
                                  <button onClick={() => adjust(primaryBrand, w, f.key, 1)} style={{ width: 22, height: 22, borderRadius: 4, border: "none", background: "#22C55E", color: WHITE, cursor: "pointer", fontSize: 13, lineHeight: 1, flexShrink: 0 }}>+</button>
                                </div>
                              </td>
                            ))}
                            <td style={{ padding: "6px 8px", textAlign: "center", fontWeight: 700 }}>{total}</td>
                            <td style={{ padding: "4px 8px", textAlign: "center" }}>
                              <button onClick={() => setEditCell({ ...primaryRow })} style={btn("#E5E7EB", NAVY)}>✏️</button>
                            </td>
                          </tr>
                        );
                      })}
                      ...
                      <tr style={{ background: "#EEF2FF", borderTop: "2px solid " + NAVY }}>
                        ...
                        {FIELDS.map(f => <td key={f.key} style={{ padding: "6px 8px", textAlign: "center", fontWeight: 700, color: f.color }}>{bt[f.key]}</td>)}
                        <td style={{ padding: "6px 8px", textAlign: "center", fontWeight: 700 }}>{Object.values(bt).reduce((a, b) => a + b, 0)}</td>
                        <td />
                      </tr>
                    </>
                  );
